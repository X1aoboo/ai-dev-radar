...
研发总览面向管理者优先展示选定分析月份的整体结论。页头的 `month` 同时驱动成熟度和月度事实，关键研发活动与通用研发能力切换共用该月份；有权限用户可从同一处进入成熟度维护。首屏摘要包括团队数、当前分类已评估能力点的领域平均成熟度、成熟度覆盖率和事实完整状态。成熟度主体使用领域平均雷达图；缺失轴不补零。

关键研发活动把 AI 渗透率/活动比率与提效率分开横向比较，并显示 `company_average`、有效团队数/总团队数及指标下钻。通用研发能力按目录实际类型展示比率、数量、布尔状态或提效率，不渲染不存在的空类型模块。选定月份没有事实时显示明确空状态，不回退到其他月份。需关注项优先显示负提效、缺失成熟度/事实和低于领域团队均值的团队-活动组合；相对差距只称“相对靠后”。
