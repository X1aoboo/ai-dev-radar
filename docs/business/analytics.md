...
研发总览面向管理者按“上下文与筛选 → Signal 摘要 → Insight 趋势卡 → 活动对照/能力指标 → 深入分析”展示选定分析月份。页头的 `month` 同时驱动成熟度和月度事实，关键研发活动与通用研发能力切换共用该月份；有权限用户可从同一处进入成熟度维护。Signal 摘要只展示数量和最高级别，详情在默认关闭的右侧抽屉中按优先级提供证据。Insight 卡展示成熟度领域平均或真实指标的当前值、环比、近六个月趋势以及 best/weak 项。当前没有目标或外部行业基准，页面不输出达标、优秀或行业排名判断。
...
完整团队×能力点矩阵默认收起，展开后包含领域平均行；桌面固定团队列，移动端保留局部横向滚动提示。周、版本、迭代筛选、完整指标比较和趋势图位于“深入分析”折叠区。团队下钻查看团队趋势；指标详情按目录类型展示率、数量柱图或布尔状态。团队、版本、迭代、时间粒度与时间字段影响对应事实查询；通用研发能力不支持迭代维度。
