...
需求工作台通过 IR、AR、SR 页签分类；页签路径可直接访问，未知分类回退 IR。IR 使用内容区 PageHeader、紧凑常用/高级筛选工具栏和以正式数据表为主的操作台；表头固定，横向滚动仅由表格在必要时承接。IR 支持筛选、分页、新增和编辑；保存时校验业务字段、层级及权限。页面编辑可显式覆盖已有字段，正式数据变更写入 AuditLog，AI 属性另记字段级来源、维护人及时间。当前没有 IR 删除接口，AR/SR 没有数据表单。
