...
`frontend/src/design/tokens.css` 是 light mode 的运行时语义 token 源，`design-system.css` 持有全局滚动条、排版、网格与共享 PageHeader/FilterToolbar 样式；`theme.js` 和 `charts/chartTheme.js` 运行时解析这些 token 供 Ant Design 与 ECharts 使用。`app.css` 暂保留兼容 alias，供未迁移的现有页面 CSS 使用；路由分期迁移完成后再删除。`components/PageHeader.jsx`、`FilterToolbar.jsx`、`MetricCard.jsx` 与 `AnalyticsPanel.jsx` 只负责呈现，不拥有路由、请求或领域状态。图表选项继续拥有各自的业务数据映射。`premium-ui.json` 为 UI 静态审计声明产品配置和已接受的原生 Select/Listbox、Date 控件归属。AnalyticsPages、DataManagement、AppShell 与 TeamDrilldown 的运行时 CSS 都已迁为语义 token 消费者；无导入点的 `overview/overview.css` 已删除，指标详情所需趋势卡样式由 AnalyticsPages 共享。DataManagementPage 的 IR 表把 `FilterToolbar` 放在 Table 之前，并由 Table 自己处理 sticky header/局部横向滚动；UsersSettingsPage、CollectionsSettingsPage 和数据管理各主工作区复用 PageHeader。登录页继续由 App 拥有认证请求，使用 `noValidate`、内联错误和字段 ARIA 关联处理空凭据。App 监听路由路径并为登录、洞察、数据管理和系统管理页面设置本地化 `document.title`。
