...
- 团队下钻页已实现 15 个活动的 KPI、活动目录锚点、团队趋势卡、指标切换、详细数据展开、迭代对比、事实记录和布尔状态展示。
- 复用总览筛选状态；时间/迭代维度、通用能力的固定近 6 个月月趋势，以及详情展开均已验证。
- 修复团队深链打开时的固定 accent 色槽位一致性，并补齐稀疏指标详情表的周期行。
- 验证：后端 79 项测试、前端 15 项测试、前端构建和真实浏览器验收通过。
