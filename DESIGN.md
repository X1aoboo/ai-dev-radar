...
- **Token ownership/runtime mapping:** `frontend/src/design/tokens.css` is the canonical runtime source. `frontend/src/design/theme.js` and `frontend/src/charts/chartTheme.js` resolve the CSS semantic tokens for Ant Design and ECharts; `app.css` retains temporary `--app-*` compatibility aliases. Shared components and chart options consume the resolved values. Phase 5 removes compatibility aliases after route migration.
